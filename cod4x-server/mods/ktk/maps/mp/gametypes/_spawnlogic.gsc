...
spawnPointInit()
{
	...
	//KILL THE KING - Fix the angles of the spawnpoints
	//For example the rotated view on mp_toujane_beta
	spawnpoint.angles = (0, spawnpoint.angles[1], 0);
...
getSpawnpoint_NearPos( spawnPoints, pos )
{
	...
	for( i = 0; i < spawnPoints.size; i++ )
	{
		tempDist = distance( spawnPoints[i].origin, pos );
		if ( tempDist < dist )
		{
			dist = tempDist;
			spawn = spawnPoints[i];
...
getSpawnpoint_NearTeamFlag(team)
{
	if(!isDefined(level.FlagSpawns[team]) || level.FlagSpawns[team].size <= 0)
	{
	...
	tempSpawn = undefined;
	tempDist = 999999999;
	for(i=0;i<level.FlagSpawns[team].size;i++)
	{
		totalDist = 0;
	
		for(j=0;j<level.players.size;j++)
		{
			if(level.players[j].pers["team"] != team)
				continue;
				
			totalDist += Distance(level.FlagSpawns[team][i].origin, level.players[j].origin);
		}
		
		if(totalDist <= tempDist)
		{
			tempDist = totalDist;
			tempSpawn = level.FlagSpawns[team][i];
		}
	}
