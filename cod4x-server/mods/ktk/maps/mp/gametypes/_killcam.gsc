...
)
{
	...
	*/
	...
	self.killcamInThirdPerson = false;
	...
	if(self isDog())
	{
		if(isDefined(self.pers["dogthirdperson"]) && self.pers["dogthirdperson"])
		{
			if(!self.killcamInThirdPerson)
				self setClientDvars("cg_thirdperson", 0, "cg_drawGun", 1);
		}
		else
		{
			if(self.killcamInThirdPerson)
...
endKillcam()
{
	...
	if ( !level.splitscreen && !level.finalcamplaying )
	{
		self hidePerk( 0 );
		self hidePerk( 1 );
		self hidePerk( 2 );
	...
	//self iPrintLnBold("cam :" + self.killcamInThirdPerson);
	//self iPrintLnBold("after :" + self.pers["thirdperson"]);
	//looks like the setClientDvar() does not work when the player is in killcam
	//so... we have to reset it on respawn
	...
	if(self isDog())
	{
		...
		{
			if(!self.killcamInThirdPerson)
				self setClientDvars("cg_thirdperson", 1, "cg_drawGun", 0);
		}
		else
		{
			if(self.killcamInThirdPerson)
