...
KingHealthHUD()
{
	...
	if(self getKtkStat(2418) == 1)
	{
		...
		self.kinghealthhud.hidewheninmenu = true;
		self.kinghealthhud.label = self GetLocalizedString("HUD_KING_HEALTH"); //&"King health: &&1";
	...
	else
	{
		...
		self.kinghealthhud.background.hidewheninmenu = true;
...
TerminatorHealthHUD()
{
	...
	if(self getKtkStat(2418) == 1)
	{
		...
		self.terminatorhealthhud.hidewheninmenu = true;
		self.terminatorhealthhud.label = self GetLocalizedString("HUD_TERMINATOR_HEALTH"); //&"Terminator health: &&1";
	...
	{	
		...
		self.terminatorhealthhud.background.hidewheninmenu = true;
		self.terminatorhealthhud.background setShader("black", level.barsize + 4, 10 + 4);
	}
}

UpdateHealthbarPlayerNotify(message, type)
{
	...
	for(i=0;i<level.players.size;i++)
	{
		...
		level.players[i] notify(message, type);
	}
}
...
MonitorHealthBars()
{
	...
	while(level.roundstarted)
	{
		self waittill("update_healthbar", type);
	
		//self iPrintLnBold("update " + type);
	
		if(type == "king")
		{
			if(isDefined(level.king) && isDefined(self.kinghealthhud))
			{
				if(self getKtkStat(2418) == 1)
				{
		...
		if(type == "assassin")
		{
			if(isDefined(level.assassin) && isDefined(self.kinghealthhud))
			{
				if(self getKtkStat(2418) == 1)
				{
					self.kinghealthhud.alpha = 0.75;
		...
		if(type == "terminator")
		{
			if(isDefined(level.terminator) && isDefined(self.terminatorhealthhud))
			{
				if(self getKtkStat(2418) == 1)
				{
...
KillstreakHud()
{
	...
	self.killstreak_hud.label = &"MPUI_STREAK_PRE"; //&"Killstreak: ^2&&1";
...
HardpointLiveTimeHud(hardpoint, timer, definition)
{
	...
	else
	{
		if(isDefined(timer) && isDefined(definition))
		{
			...
			{
				if(hardpoint == level.ktkWeapon["rccar"]) 			self.hardpointTime_hud.label = self GetLocalizedString("HARDPOINT_TIMER_RCXD"); //&"RC-XD explodes in: ^2&&1";
				else if(hardpoint == level.ktkWeapon["ac130"]) 		self.hardpointTime_hud.label = self GetLocalizedString("HARDPOINT_TIMER_AC130"); //&"AC130 leaves in: ^2&&1";
				else if(hardpoint == level.ktkWeapon["predator"]) 	self.hardpointTime_hud.label = self GetLocalizedString("HARDPOINT_TIMER_PREDATOR"); //&"Predator explodes in: ^2&&1";
				else if(hardpoint == level.ktkWeapon["sentrygun"]) 	self.hardpointTime_hud.label = self GetLocalizedString("HARDPOINT_TIMER_SENTRY"); //&"Sentry Gun explodes in: ^2&&1";
				else if(hardpoint == "helicopter_mp") 				self.hardpointTime_hud.label = self GetLocalizedString("HARDPOINT_TIMER_HELICOPTER"); //&"Helicopter leaves in: ^2&&1";
...
propsControls()
{
	...
	for(i=0;i<4;i++)
	{	
		if(isDefined(self.propsControlHud[i]))
			self.propsControlHud[i] destroy();
	
		self.propsControlHud[i] = newclientHudElem(self);
		self.propsControlHud[i].font = "default";
		self.propsControlHud[i].fontScale = 1.4;
		self.propsControlHud[i].alignX = "left";
		self.propsControlHud[i].alignY = "bottom";
		self.propsControlHud[i].horzAlign = "left";
		self.propsControlHud[i].vertAlign = "top";
		self.propsControlHud[i].x = 110;
		self.propsControlHud[i].y = 65 + 15*i;
		self.propsControlHud[i].alpha = 1;
		self.propsControlHud[i].sort = 1;
		self.propsControlHud[i].archived = false;
		self.propsControlHud[i].foreground = true;
		self.propsControlHud[i].hidewheninmenu = true;
		
		switch(i)
		{
			case 0: self.propsControlHud[i].label = &"Rotate: (^3[{+smoke}]^7, ^3[{+leanleft}]^7, ^3[{+leanright}]^7)"; break;
			case 1: self.propsControlHud[i].label = &"Auto Rotate: ^3[{+frag}]"; break;
			case 2: self.propsControlHud[i].label = &"Reset: ^3[{+attack}]"; break;
			case 3: self.propsControlHud[i].label = &"Direction of rotation: ^3[{+activate}]"; break;
			default: break;
		}
	}
}
...
RemoveCustomHuds(type)
{
	...
	if(isDefined(self.propsControlHud))
	{
		for(i=0;i<4;i++)
		{	
			if(isDefined(self.propsControlHud[i]))
