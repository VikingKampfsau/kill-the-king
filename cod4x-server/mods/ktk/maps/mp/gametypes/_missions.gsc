...
initMissionData()
{
	self.pers[level.ktkWeapon["rccar"]] = 0;
	self.pers["airstrike_mp"] = 0;
	self.pers["helicopter_mp"] = 0;
	self.pers[level.ktkWeapon["mortars"]] = 0;
	self.pers[level.ktkWeapon["ac130"]] = 0;
	self.pers[level.ktkWeapon["sentrygun"]] = 0;
	self.pers[level.ktkWeapon["predator"]] = 0;
	self.pers[level.ktkWeapon["nuke"]] = 0;
	self.pers[level.ktkWeapon["juggernaut"]] = 0;
...
challengeNotify( challengeName, challengeDesc )
{
	...
	if(!IsString(challengeName))
		notifyData.notifyText = challengeName;	
	else
		notifyData.notifyText = self GetLocalizedString(challengeName);	
...
ch_hardpoints( data )
{
	if(!isDefined(data) || !isDefined(data.hardpointType) || !isDefined(data.player) || !isPlayer(data.player))
      return;
	...
	if ( data.hardpointType == level.ktkWeapon["rccar"] )
	{
	...
	else if ( data.hardpointType == level.ktkWeapon["mortars"] )	
	{
		player processChallenge( "ch_ktk_mortar" );

		if ( player.pers[data.hardpointType] >= 2 )
			player processChallenge( "ch_ktk_mortar" );
	}
	else if ( data.hardpointType == level.ktkWeapon["ac130"] )	
	{
		player processChallenge( "ch_ktk_ac130" );
...
ch_kills( data, time )
{
	...
	if(!isDefined(data.victim.pers["team"]) || data.victim.pers["team"] == "spectator")
		return;
	...
	if(data.victim isKing())
	{
		if(int((maps\mp\gametypes\_globallogic::getTimeRemaining() / 1000)) <= 10)
			player processChallenge( "ch_ktk_countdown" );
	}

	if(data.sWeapon == level.ktkWeapon["poisongas"])
		player processChallenge( "ch_ktk_breathless" );

	if(data.victim isInLastStand() && (data.sWeapon == level.ktkWeapon["knife"] || data.sMeansOfDeath == "MOD_MELEE"))
		player processChallenge( "ch_ktk_brokenconvention" );
		
	if(data.victim isReviving())
		player processChallenge( "ch_ktk_nomercy" );

	if(isDefined(data.victim.droppedparachute) && !data.victim.droppedparachute)
		player processChallenge( "ch_ktk_deadbird" );

	if(GetTeamPlayersAlive(game["defenders"]) == 1 && player isKing() && player.cur_kill_streak == 10)
		player processChallenge( "ch_ktk_lastbastion" );
	
	if(isDefined(data.victim.pers["vip"]) && data.victim.pers["vip"])
		player processChallenge( "ch_ktk_hitman_" );
		
	if(data.victim isTerminator())
		player processChallenge( "ch_ktk_destroyer" );
	
	if(data.victim isAGuard() && player.health == player.maxhealth && data.sWeapon == level.ktkWeapon["knife"])
		player processChallenge( "ch_ktk_perfectassassination" );
	
	if(data.sWeapon == level.ktkWeapon["knife"] || data.sMeansOfDeath == "MOD_MELEE")
		player processChallenge( "ch_ktk_jack_" );
	...
	if ( level.teamBased )
	{
		...
		if(player isMainAssassin() && player.killedPlayersCurrent.size >= level.playerCount[data.victim.pers["team"]] )
		player processChallenge( "ch_ktk_army" );
	}
	...
	else if ( isSubStr( data.sMeansOfDeath, "MOD_GRENADE" ) || isSubStr( data.sMeansOfDeath, "MOD_EXPLOSIVE" ) || isSubStr( data.sMeansOfDeath, "MOD_PROJECTILE" ) )
	{
		...
		if ( isDefined( data.victim.explosiveInfo["damageTime"] ) && data.victim.explosiveInfo["damageTime"] == time )
		{
			...
			if ( data.sWeapon == level.ktkWeapon["rccar"] )
			{
				if(player.pers["bulletStreak"] > 2 || player.explosiveKills[expId] > 2)
					player processChallenge( "ch_ktk_driver" );	
			}
			...
			else if ( isStrStart( data.sWeapon, "c4_" ) )
			{
				...
				if ( player.explosiveKills[expId] > 2 )
					player processChallenge( "ch_ktk_sticky" );
			...
			else if ( isStrStart( data.sWeapon, "claymore_" ) )
			{
				...
				player processChallenge( "ch_ktk_ied_" );
			...
			else if ( data.sWeapon == "destructible_car" )
			{
				...
				if(data.victim isMainAssassin())
					player processChallenge( "ch_ktk_carbomb" );
			}
	...
	else if ( isSubStr( data.sMeansOfDeath,	"MOD_MELEE" ) )
	{
		...
		if ( abs(angleDiff) < 30 )
		{
			...
			player processChallenge( "ch_ktk_backstabber_" );
		}
	}
...
ch_bulletDamageCommon( data, player, time, weaponClass )
{
	...
	if ( player.pers["bulletStreak"] == 2 && ( weaponClass == "weapon_assault" || weaponClass == "weapon_smg" || weaponClass == "weapon_lmg" || weaponClass == "weapon_mg" || weaponClass == "weapon_sniper" ) )
		player processChallenge( "ch_ktk_2in1" );
	...
	if ( isDefined(data.victim.iDFlagsTime) && data.victim.iDFlagsTime == time )
	{
...
ch_roundwin( data )
{
	...
	switch ( level.gameType )
	{
		case "ktk":
			if(player isAGuard())
			{
				player processChallenge( "ch_ktk_legend_" );
				player processChallenge( "ch_ktk_guardforlife" );

				if(GetTeamPlayersAlive(game["defenders"]) == 1)
					player processChallenge( "ch_ktk_hideout" );
			}
			break;
		case "ctf":
		case "dom":
	...
	while(1)
	{
		...
		if ( !self isOnGround() )
		{
			...
			if ( falldist / 12.0 > 30 && !isAlive( self ) )
			{
				...
				if(isDefined(self.droppedparachute) && self.droppedparachute)
					self processChallenge( "ch_ktk_hardlanding" );
			}
			
			//println( "You fell ", falldist / 12.0, " feet");
			
			if( self.health < oldhealth && self.health >= oldhealth - 10)
				self processChallenge( "ch_ktk_falldamage" );
		}
		wait .05;
	}
}
...
lastManSD()
{
	...
	self processChallenge( "ch_ktk_bodyguard" );
}
...
FinalKill(sMeansofDeath, sWeapon)
{
	self processChallenge( "ch_ktk_fame" );
	
	if(sMeansofDeath == "MOD_HEAD_SHOT")
		self processChallenge( "ch_ktk_headhunter" );
		
	if(sWeapon == level.ktkWeapon["throwingKnife"])
		self processChallenge( "ch_ktk_throwknife" );
}

GunGame()
{
	self processChallenge( "ch_ktk_weaponnomatter" );
	
	if(self GetCurrentWeapon() == level.GunGameWeapons[1] || (self getKtkStat(2363) == 1 && self GetCurrentWeapon() == level.GunGameWeapons[2]))
		self processChallenge( "ch_ktk_sharpshooter" );
}

Medic()
{
	self processChallenge( "ch_ktk_medic_");
}
...
monitorPlayTime()
{
	...
	while(1)
	{
		wait 30; //don't count it as playtime when the player does not stay for minimum 30 seconds
		self processChallenge("ch_ktk_regularplayer_", 30);
	}
}