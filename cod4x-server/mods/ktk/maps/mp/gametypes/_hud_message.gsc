...
BreakOnRoundEnd(notifyData)
{
	...
	wait .05;
		...
		if(isDefined(notifyData) && level.gameEnded)
		{
			self notify ( "notifyMessageDone" );
			resetNotify();
			
			if(isDefined(notifyData.sound))
				self StopLocalSound( notifyData.sound );
			if(isDefined(notifyData.leaderSound))
				self StopLocalSound( game["voice"][self.pers["team"]]+game["dialog"][notifyData.leaderSound] );
	...
	//why did i add this? it will end the function!
	//self endon("notifyMessageDone");

	self notify("notifyMessageCleaner");
	self endon("notifyMessageCleaner");
...
showNotifyMessage( notifyData )
{
	...
	self thread BreakOnRoundEnd(notifyData);
...
outcomeNotify( winner, endReasonText )
{
	...
	/* KILL THE KING -> I REALLY CANT REMEMBER WHAT THIS WAS FOR - SO - REMOVED
	if ( isDefined( players[1] ) && players[0].timeonfire == players[1].timeonfire && (self == players[0] || self == players[1]) )
	{
		outcomeTitle setText( game["strings"]["tie"] );
		outcomeTitle.glowColor = (0.2, 0.3, 0.7);
	}
	else if ( isDefined( players[2] ) && players[0].timeonfire == players[2].timeonfire && self == players[2] )
	{
		outcomeTitle setText( game["strings"]["tie"] );
		outcomeTitle.glowColor = (0.2, 0.3, 0.7);
	}
