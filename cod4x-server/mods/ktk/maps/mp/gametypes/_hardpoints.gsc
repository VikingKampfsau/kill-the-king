...
init()
{
	precacheItem( level.ktkWeapon["rccar"] );
	...
	level.hardpointHints[level.ktkWeapon["rccar"]] = &"MP_EARNED_RADAR";
	...
	level.hardpointInforms[level.ktkWeapon["rccar"]] = "mp_killstreak_radar";
	...
	precacheString( level.hardpointHints[level.ktkWeapon["rccar"]] );	
	...
	game["dialog"][level.ktkWeapon["rccar"]] = "uavrecon";
...
losRadiusDamage(pos, radius, max, min, owner, eInflictor)
{
	...
	for (i = 0; i < ents.size; i++)
	{
		...
		//KILL THE KING
		if(isDefined(owner) && isPlayer(owner) && owner isAGuard())
...
callStrike_bombEffect( plane, launchTime, owner, requiredDeathCount )
{
	...
	if(!isDefined(owner) || !owner isAGuard())
		return;
	...
	//removed the if because the napalm got taken out
	//if(isDefined(owner.hardpointtype) && owner.hardpointtype == "airstrike_mp")
	...
	for( i = 0; i < repeat; i++ )
	{
		...
		if(isdefined(owner.hardpointtype) && owner.hardpointtype == "napalm_mp")
			playfx( level.napalm_airstrikefx, tracehit );
...
giveHardpointItemForStreak()
{
	...
	{
		if ( streak == 3 )
			self giveHardpoint( level.ktkWeapon["rccar"], streak );
		else if ( streak == 5 )
...
//Kill the King
//i am not sure but i think the takeall() within the weaponloadout removes them again
//that's why i have added LoadOutPreviouslyGainedHardpoints() to the weaponloadout
giveHardpointItems()
{
	if ( level.gameEnded )
		return;
	...
	for(i=1;i<5;i++)
	{
		...
		if(isDefined(self.pers["actionslot"][i]["weapon"]) && self.pers["actionslot"][i]["weapon"] != "")
...
hardpointItemWaiter()
{
	...
	self thread giveOwnedHardpointItem(); //why not threaded?
	...
	for ( ;; )
	{
		...
		if( currentWeapon == level.ktkWeapon["rccar"] ||
			currentWeapon == level.ktkWeapon["mortars"] ||
			currentWeapon == level.ktkWeapon["ac130"] ||
			currentWeapon == level.ktkWeapon["nuke"] ||
			currentWeapon == level.ktkWeapon["predator"] ||
			currentWeapon == level.ktkWeapon["sentrygun"] ||
			currentWeapon == level.ktkWeapon["juggernaut"] ||
			currentWeapon == "helicopter_mp" ||
			currentWeapon == "airstrike_mp" ||
			currentWeapon == "napalm_mp" ||
			currentWeapon == "marker_mp" )
		{
			if(self canUseHardpoint(currentWeapon))
			{
				HardpointSlot = self GetSlotForHardpoint(currentWeapon);
				
				if ( self triggerHardpoint( currentWeapon, self.prevweapon ) )
				{
					logString( "hardpoint: " + currentWeapon );

					if(currentWeapon != "marker_mp")
					{
						self thread maps\mp\gametypes\_missions::useHardpoint(currentWeapon);
						self thread [[level.onXPEvent]]( "hardpoint" );
						self.pers["hardPointItem"] = undefined;
						self ResetHardpointSlot(HardpointSlot);

						//somehow the ammo for the marker gets deleted after airstrike and mortar
						//i dont know why so i added this little workaround
						if(self HasWeapon("marker_mp"))
							self giveMaxAmmo("marker_mp");
					}
				}
			}
			...
			{
				if(self.prevweapon != "none" && currentWeapon != "marker_mp")
				{
					if(self isABot())
...
triggerHardPoint( hardpointType, lastWeapon )
{
	if(!isDefined(level.roundstarted) || !level.roundstarted)
		return false;

	if( hardpointType == level.ktkWeapon["nuke"] )
		self thread maps\mp\gametypes\_nuke::LaunchNuke();
	else if ( hardpointType == level.ktkWeapon["rccar"] )
	{
		//check if the position is ok
		trace = BulletTrace(self.origin + (0,0,60), self.origin + (0,0,10), false, self);

		if(trace["fraction"] != 1)
		{
			scr_iPrintlnBold("RCXD_NOTAVAILABLE", self);
			return false;
		}
		...
		if(self getKtkStat(2399) == 0)
			self thread maps\mp\gametypes\_rccar::SpawnCar();
		else
		{
			if(isDefined(self.rc_heli) || !VehicleCanSpawn())
			{
				scr_iPrintlnBold("RCXD_NOTAVAILABLE", self);
				return false;
			}
			
			self thread maps\mp\gametypes\_rchelicopter::SpawnRCHelicopter();
		}
	}
	else if ( hardpointType == "airstrike_mp" || hardpointType == "napalm_mp" )
	{
	...
	else if ( hardpointType == "helicopter_mp" )
	{
		...
		if(isDefined(level.chopper) || isdefined(level.HelicopterGunner))
		{
			self iPrintLnBold( level.hardpointHints[hardpointType+"_not_available"] );
			return false;
		}
	
		if(getDvarInt("scr_hardpoint_helicoptertype") == 1)
		{
			if(	isDefined(level.ah6) ||
				self isInRCToy() ||
				self isInParachute() ||
				self isCarryingExpBolt() ||
				self isInLastStand() ||
				...
				self isTerminator() ||
				self isInAC130() ||
				isDefined(level.HelicopterGunner) )
				{
					self iPrintLnBold( level.hardpointHints[hardpointType+"_not_available"] );
					return false;
				}
		...
		if(isDefined(level.heli_paths))
		{
			...
			if(isDefined(level.heli_paths[destination]))
			{
		...
		if(getDvarInt("scr_hardpoint_helicoptertype") == 0)
			...
			self thread maps\mp\gametypes\_manned_helicopter::spawnHeli(startnode);
	}
	else if ( hardpointType == level.ktkWeapon["mortars"] )
	{
		if ( isDefined( level.mortarsInUse ) )
		{
			scr_iPrintlnBold("MORTAR_NOTAVAILABLE", self);
			return false;
		}
		...
		thread LaunchMortars(self, self.pers["team"], target["position"]);
	}
	else if ( hardpointType == level.ktkWeapon["ac130"] )
	{
		if(	self isInRCToy() ||
			self isInParachute() ||
			self isCarryingExpBolt() ||
			...
			self isTerminator() ||
			self isInMannedHelicopterTurret() ||
			isDefined(level.ac130Player))
			{
				scr_iPrintlnBold("AC130_NOTAVAILABLE", self);
				return false;
			}

		thread maps\mp\gametypes\_ac130::ac130_attachPlayer(self);
	}
	else if( hardpointType == "marker_mp" )
	{
		if(	self isInRCToy() ||
			self isInParachute() ||
			self isCarryingExpBolt() ||
		...
		self thread maps\mp\gametypes\_carepackage::WaitForSupplyCall(lastWeapon);
	}
	else if( hardpointType == level.ktkWeapon["predator"] )
	{
		if(	self isInRCToy() ||
			self isInParachute() ||
			self isCarryingExpBolt() ||
			...
			self isInAC130() )
			{
				scr_iPrintlnBold("Predator_NOTAVAILABLE", self);
				return false;
			}
		
		self thread maps\mp\gametypes\_predator::LaunchPredator();
	}
	else if( hardpointType == level.ktkWeapon["sentrygun"] )
	{
		if(	self isInRCToy() ||
			self isInParachute() ||
			self isCarryingExpBolt() ||
			...
			self isInMannedHelicopterTurret() ||
			self isInAC130() ||
			level.Sentrys >= level.SentrysMax)
			{
				scr_iPrintlnBold("SENTRY_NOTAVAILABLE", self);
				return false;
			}
		
		self thread maps\mp\gametypes\_sentrygun::WaitForSentryDeploy(false, undefined, 0);
	}
	else if( hardpointType == level.ktkWeapon["juggernaut"] )
	{
		if(	self isInRCToy() ||
			self isInParachute() ||
			self isCarryingExpBolt() ||
			...
			self isInAC130())
			{
				scr_iPrintlnBold("AWARD_JUGGER_NOT_READY", self);
				return false;
			}
...
//KILL THE KING - HERE STARTS THE MORTAR SCRIPT
getTarget()
{
	self endon( "stop_location_selection" );
	self endon("death");
	...
	trace = BulletTrace(pos+(0,0,2000), pos-(0,0,2000), false, undefined);
	
	if(isDefined(trace) && (!isDefined(level.mortarsInUse) || !level.mortarsInUse))	
		return trace;
}

LaunchMortars(owner, team, FireTarget)
{
	level.mortarsInUse = true;
	//maps\mp\gametypes\_globallogic::leaderDialog( "airstrike_inbound", team );
	//maps\mp\gametypes\_globallogic::leaderDialog( "enemy_airstrike_inbound", level.otherTeam[team] );
	...
	for(i=0;i<3;i++)
	{
		...
		{
			target = FireTarget;
			target += (randomIntRange(-300,300), randomIntRange(-300,300), 0);
			
			FireVector = vectorNormalize(target - (level.MapCenter[0], level.MapCenter[1], target[2]));
			FirePos = target - FireVector * 10000;
		
			shell = spawn("script_model", FirePos);
			shell.origin = FirePos;
			shell.angles = (-45,vectorToAngles(FireVector)[1],0);
			shell setModel("projectile_hellfire_missile");

			wait randomfloat(1) + 0.25;
			
			if(isDefined(owner) && owner isAGuard())
				shell thread FireShell(FirePos, 10000, target, owner);
		}
	wait 2;
	}
	
	thread finishMortarStrike();
}

finishMortarStrike()
{
	wait 8;
	level.mortarsInUse = undefined;
}

FireShell(firepos, firedist, target, owner)
{
	self endon("death");
	...
	//don't make them all fly the same path (height)
	pitch += randomintrange( -3, 3 );
	
	range = distance2d(firepos, target);
	gravity = GetDvarInt("g_gravity");
	velocity = sqrt((gravity * range) / sin(-2 * pitch));
	flytime = (2 * velocity * sin(-1*pitch)) / gravity;
	
	if(flytime < 2) flyTime = 2;
	
	//maxheight = self.origin[2] + velocity * sin(-1*pitch) * (flytime/2) - ((gravity * sqr(flytime/2)) / 2);
	
	playfxontag(level._effect["mortartrail"], self, "tag_fx");
	self thread PlayShellFlySound(flytime);
	
	cur_time = 0;
	//lastorigin = self.origin;
	startheight = self.origin[2];
	distance = CalcDifVector(firepos, target);
	
	while(isDefined(self))
	{
		next = firepos;
		height = startheight + velocity * sin(-1*pitch) * cur_time - ((gravity * sqr(cur_time)) / 2);
		
		if(next[0] >= 0)
		{
			if(next[1] >= 0)
				next -= ((distance[0], distance[1], 0) * cur_time / flytime);
			else
				next -= ((distance[0], -1 * distance[1], 0) * cur_time / flytime);
		}
		else if(next[0] < 0)
		{
			if(next[1] < 0)
		...
		self MoveTo(next, .1);
		self RotateTo(vectorToAngles(next - self.origin), .1);
		
		if(cur_time >= (flytime/2))
		{
			trace = BulletTrace(self.origin, next, false, self);
			if(trace["fraction"] != 1 && trace["surfacetype"] != "default")
			{
				finalPos = trace["position"];
				self moveto(finalPos, .1);
				wait .1;
				self thread ShellExplosion(owner, trace["surfacetype"], finalPos);
				break;
			}
		}
...
ShellExplosion(owner, surface, endorigin)
{
	switch(surface)
	{
		case "bark": sound = "rocket_explode_bark"; effect = level._effect["mortarexplosion_wood"]; break;
		case "brick": sound = "rocket_explode_brick"; effect = level._effect["mortarexplosion_brick"]; break;
		case "carpet": sound = "rocket_explode_carpet"; effect = level._effect["mortarexplosion_wood"]; break;
		case "cloth": sound = "rocket_explode_cloth"; effect = level._effect["mortarexplosion_default"]; break;
		case "concrete": sound = "rocket_explode_concrete"; effect = level._effect["mortarexplosion_concrete"]; break;
		case "dirt": sound = "rocket_explode_dirt"; effect = level._effect["mortarexplosion_dirt"]; break;
		case "flesh": sound = "rocket_explode_flesh"; effect = level._effect["mortarexplosion_default"]; break;
		case "foliage": sound = "rocket_explode_foliage"; effect = level._effect["mortarexplosion_dirt"]; break;
		case "glass": sound = "rocket_explode_glass"; effect = level._effect["mortarexplosion_glass"]; break;
		case "grass": sound = "rocket_explode_grass"; effect = level._effect["mortarexplosion_dirt"]; break;
		case "gravel": sound = "rocket_explode_gravel"; effect = level._effect["mortarexplosion_dirt"]; break;
		case "ice": sound = "rocket_explode_ice"; effect = level._effect["mortarexplosion_snow"]; break;
		case "metal": sound = "rocket_explode_metal"; effect = level._effect["mortarexplosion_metal"]; break;
		case "mud": sound = "rocket_explode_mud"; effect = level._effect["mortarexplosion_dirt"]; break;
		case "paper": sound = "rocket_explode_paper"; effect = level._effect["mortarexplosion_default"]; break;
		case "rock": sound = "rocket_explode_rock"; effect = level._effect["mortarexplosion_rock"]; break;
		case "sand": sound = "rocket_explode_sand"; effect = level._effect["mortarexplosion_dirt"]; break;
		case "snow": sound = "rocket_explode_snow"; effect = level._effect["mortarexplosion_snow"]; break;
		case "water": sound = "rocket_explode_water"; effect = level._effect["mortarexplosion_water"]; break;
		case "wood": sound = "rocket_explode_wood"; effect = level._effect["mortarexplosion_wood"]; break;
		case "asphalt": sound = "rocket_explode_asphalt"; effect = level._effect["mortarexplosion_plaster"]; break;
	
		default: sound = "rocket_explode_default"; effect = level._effect["mortarexplosion_default"]; break;
	}
	...
	else
	{
		if(game["customEvent"] == "kingvsking")
			self RadiusDamage(self.origin, 355 , 1000 , 300, owner, "MOD_EXPLOSIVE", level.ktkWeapon["mortars"]);
		else
		{
			if(owner isAnAssassin())
				self RadiusDamage(self.origin, 0 , 0 , 0);
			else
				self RadiusDamage(self.origin, 355 , 1000 , 300, owner, "MOD_EXPLOSIVE", level.ktkWeapon["mortars"]);
		}
