...
init()
{
	...
	for( j = 0; j <= max_weapon_num; j++ )
	{
		if(getdvarInt("weap_reverse_load_" + getDvar("mapname")) == 1)
			i = max_weapon_num - j;
		else
			i = j;
	...
	//KILL THE KING
	//Precaching only the weapons we really use
	maps\mp\gametypes\_precache::precacheWeaponSelection();
...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		if(!self isABot())
		{
			...
			self thread watchMeleeUsage();
			
			self thread maps\mp\gametypes\_throwingknife::CheckForKnifeThrow();
			self thread maps\mp\gametypes\_javelin::WaitForJavelinPullout();
			self thread maps\mp\gametypes\_tripwire::CheckDeploy();
		}
...
watchMeleeUsage()
{
	...
	while(1)
	{
		...
		{
			if(self getCurrentWeapon() == level.ktkWeapon["knife"] || self getCurrentWeapon() == "defaultweapon_mp" || self getCurrentWeapon() == level.ktkWeapon["katana"] || self getCurrentWeapon() == level.ktkWeapon["riotshield"])
			{
				self maps\mp\gametypes\_misc::ExecClientCommand("+melee;-melee");
...
isMeleeing()
{
	...
	while(1)
	{
		...
		{
			self thread maps\mp\gametypes\_knifeassist::activateKnifeAssist();
			wait 1.5;
...
watchWeaponChange()
{
	...
	if(self.lastDroppableWeapon == level.ktkWeapon["riotshield"])
		self.leftWeaponWorldModel = "worldmodel_riot_shield_iw5";
	else if(self.lastDroppableWeapon == level.ktkWeapon["knife"])
		self.leftWeaponWorldModel = "worldmodel_knife";
	
	if(isDefined(self.leftWeaponWorldModel))
		self attach(self.leftWeaponWorldModel, "tag_weapon_left", false);
	...
	while(1)
	{
		...
		if(isDefined(self.leftWeaponWorldModel) && self hasAttached(self.leftWeaponWorldModel))
		{
			self detach(self.leftWeaponWorldModel, "tag_weapon_left");
			self.leftWeaponWorldModel = undefined;
		}

		if(newWeapon != "none")
		{
			if(newWeapon == level.ktkWeapon["crossbow"])
				self thread maps\mp\gametypes\_escaperope::escapeRopeMonitor();
			else if(newWeapon == level.ktkWeapon["riotshield"])
				self.leftWeaponWorldModel = "worldmodel_riot_shield_iw5";
			else if(newWeapon == level.ktkWeapon["knife"])
				self.leftWeaponWorldModel = "worldmodel_knife";
			
			if(isSniper(newWeapon))
				self thread maps\mp\gametypes\_thermal::ThermalScope(newWeapon);
			...
			if(self isAnAssassin())
			{
				...
				if(!self hasAllowedPlayerWeapon(newWeapon))
				{
					if(self isDog() || game["customEvent"] == "dogfight")
							defaultWeapon = "defaultweapon_mp";
					else if(self isMainAssassin())
					{
						if(!self HasWeapon(level.assassin_primary))
							defaultWeapon = level.assassin_primary;
						else if(!self HasWeapon(level.assassin_secondary))
							defaultWeapon = level.assassin_secondary;
						else if(!self HasWeapon(level.ktkWeapon["crossbowExp"]))
							defaultWeapon = level.ktkWeapon["crossbowExp"];
					}
					else
					{
						...
						else
						{
							if(self.assassintype == "katana")
								defaultWeapon = level.ktkWeapon["katana"];
							else if(self.assassintype == "knife")
								defaultWeapon = level.ktkWeapon["knife"];
							else if(self.assassintype == "bolt")
								defaultWeapon = level.ktkWeapon["mosinnagat"];
							else if(self.assassintype == "scope")
								defaultWeapon = "remington700_mp";
							else if(self.assassintype == "strongscope")
								defaultWeapon = "m40a3_acog_mp";
							else if(self.assassintype == "hardscope")
								defaultWeapon = level.ktkWeapon["intervention"];
						}
					}
					
					if(isSubStr(newWeapon, "colt45") || isSubStr(newWeapon, "deserteagle"))
					{
						maps\mp\gametypes\_misc::DebugMessage(1, "Player " + self.name + " (" + self.pers["team"] + ") switched to " + newWeapon);
					
						if(!isDefined(defaultWeapon))
							maps\mp\gametypes\_misc::DebugMessage(5, "His defaultWeapon is not defined");
							
						if(!isDefined(self.assassintype))
							maps\mp\gametypes\_misc::DebugMessage(5, "His assassintype is not defined");
					}

					if(isDefined(defaultWeapon))
					{
						self dropItem( newWeapon );
...
hasAllowedPlayerWeapon(weapon)
{
	...
	else
	{
		...
		if(game["customEvent"] == "alien" && weapon == level.ktkWeapon["disruptor"])
			return true;
			
		if(game["customEvent"] == "terror" && weapon == level.ktkWeapon["c4bomb"])
			return true;
	
		if(getDvarInt("scr_mod_weaponbox") == 1 && isDefined(level.BoxAssassinWeapons))
		{
			for(i=0;i<level.BoxAssassinWeapons.size;i++)
			{
				if(!isDefined(level.BoxAssassinWeapons[i]))
					break;
					
				if(weapon == level.BoxAssassinWeapons[i])
					return true;
			}
		}
	
		if(isDefined(self.weaponCpAssassins) && weapon != self.weaponCpAssassins)
			return false;
	
		if(	weapon != level.assassin_secondary && 
			weapon != level.assassin_primary && 
			weapon != level.ktkWeapon["katana"] &&
			weapon != level.ktkWeapon["mosinnagat"] &&
			weapon != level.ktkWeapon["magnum"] &&
			weapon != level.ktkWeapon["javelin"] &&
			weapon != "ladderclimb_mp" &&
			weapon != "remington700_mp" &&
			weapon != "m40a3_acog_mp" &&
			weapon != "marker_mp" &&
			weapon != level.ktkWeapon["knife"] &&
			weapon != level.ktkWeapon["intervention"] &&
			weapon != level.ktkWeapon["m21Silencer"] &&
			weapon != level.ktkWeapon["crossbowExp"] &&
			weapon != "ladderclimb_mp")
			return false;
	}
...
hasScope( weapon )
{
	if ( isSubStr( weapon, "_acog_" ) )
		return true;
	if ( weapon == level.ktkWeapon["m21Silencer"] )
		return true;
	if ( weapon == "aw50_mp" )
		return true;
	...
	if ( weapon == "dragunov_mp" )
		return true;
	if ( weapon == level.ktkWeapon["intervention"] )
		return true;
...
dropOffhand()
{
	...
	//grenadeTypes[grenadeTypes.size] = level.ktkWeapon["throwingKnife"];
	// no dropping of secondary grenades.
	//grenadeTypes[grenadeTypes.size] = level.ktkWeapon["poisongas"];
	//grenadeTypes[grenadeTypes.size] = "flash_grenade_mp";
	//grenadeTypes[grenadeTypes.size] = level.ktkWeapon["syrette"];
...
getFragGrenadeCount()
{
	grenadetype = level.ktkWeapon["throwingKnife"];
...
{
	grenadetype = level.ktkWeapon["poisongas"];
...
watchCurrentFiring( curWeapon )
{
	...
	if(statTotal > 0 && statHits > 0)
	{
		float = int(100/(int((statTotal/statHits)*100)/100)*100)/100; //accuracy in percent :)
		helpstring = "" + float + "";
		
		accuracy = [];
		accuracy = strTok( helpstring, "." );
	}
	else
	{
		accuracy = [];
	...
	if(!isDefined(accuracy[1]))
		accuracy[1] = 0;
	
	self maps\mp\gametypes\_persistence::statSet( "accuracy", int(accuracy[0]) );
	self maps\mp\gametypes\_persistence::statSet( "accuracy_2", int(accuracy[1]) );
/*
...
watchGrenadeUsage()
{
	...
	// Kill the King - Disabled Grenade Throwback
...
{/*
	...
	while(1)
	{
		...
		if ( weapname == "c4" || weapname == level.ktkWeapon["crossbowExp"] )
		{
...
watchClaymores()
{
	...
	while(1)
	{
		...
		if ( weapname == "claymore" || weapname == "claymore_mp" )
		{
			...
			claymore thread OwnerClaymorePickup();
...
OwnerClaymorePickup()
{
	...
	trigger = spawn("trigger_radius", self.origin, 0, level.claymoreDetonateRadius/2, level.claymoreDetonateRadius);
	...
	while(1)
	{
		...
		if(isdefined(self.owner) && player == self.owner && player UseButtonPressed())
		{
			if(player HasWeapon("claymore_mp"))
				player setWeaponAmmoClip("claymore_mp", player GetAmmoCount("claymore_mp")+1);
			else
			{
				player giveWeapon("claymore_mp");
				player setWeaponToSlot("claymore_mp");
...
watchC4Detonation()
{
	...
	while(1)
	{
		...
		if ( weap == level.ktkWeapon["crossbowExp"] )
		{
			newarray = [];
			for ( i = 0; i < self.c4array.size; i++ )
			{
